...
import com.yuyang.baiduguiji.util.GetBMIValuesHelper;
...
public class PlanningActivity extends BaseActivity implements View.OnFocusChangeListener,View.OnClickListener{
    ...
    protected void initValues() {
        ...
        //提供Map 集合存储 从 GetBMIValuesHelper工具类中获取体重的最大值和最小值，通过最大值 最小值计算健康体重的范围
    ...
    protected void setViewsListener() {
        ...
        setPlanWeight.setValueChangeListener(set_plan_weight_listener);  //ScalRulerView 注册值变监听
    }
    ...
    protected void setViewsFunction() {
        ...
        //目标体重选择
        setPlanWeight.initViewParam(now_weight, 130, 30);  //ScalRulerView  设置值范围

        //创建时间选择器 初始 为当前日期 开始时间后   跟据isSetStart 判断为创建开始或结束时间表
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.finish://完成
                //当体重开始时间 和结束时间都设置好了之后 才可以点击完成按钮跳转到FuncationActivity
                ...
                if (plan_want_weight < min_normal_weight){
                    ...
                    if ((!getString(R.string.set_start_time).equals(start_dates))&&(!getString(R.string.set_stop_time).equals(stop_dates))){
                        if ((start_date <= stop_date) && (start_year <= stop_year) && (start_month <= stop_month)){
                            //信息全部符合规则 将信息进行保存后进行活动的跳转
                            saveCustomSettingValues(start_dates,stop_dates);  //执行这个方法将所 合适体重 目标体重 开始结束时间进行了保存
